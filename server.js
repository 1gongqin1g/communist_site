...
app.post('/delete-account', (req, res) => {
  ...
  if (!userId) return res.json({ error: '未登录' });
  ...
  data.groups.forEach(g => {
    if (g.members.includes(userId)) {
      ...
      if (g.ownerId === userId) {
        if (g.members.length > 0) g.ownerId = g.members[0];
      }
    }
  });
...
app.get('/all-my-articles', (req, res) => res.json(data.articles || []));
...
app.post('/publish', (req, res) => {
  ...
  const article = {
    id: generateUniqueId(), authorId: userId, author: user.nickname,
    title, content, tags, likes: 0, favorites: 0, coins: 0, recommends: 0, timestamp: Date.now()
  };
...
app.delete('/articles/:id', (req, res) => {
  const userId = req.headers['x-user-id'];
  const article = data.articles.find(a => a.id === req.params.id);
  if (!article || article.authorId !== userId) return res.json({ error: '无权限' });
...
app.post('/publish-public', (req, res) => {
  ...
  if (!tags || tags.length === 0) return res.json({ error: '请至少选择一个标签' });
  data.publicArticles.push({ id: generateUniqueId(), author: '匿名同志', title, content, tags, timestamp: Date.now() });
  saveData(); res.json({ success: true });
});
...
app.post('/publish-fulltext', (req, res) => {
  ...
  if (!tags || tags.length === 0) return res.json({ error: '请至少选择一个标签' });
  data.fulltextArticles.push({ id: generateUniqueId(), author: user.nickname, title, content, tags, recommends: 0, timestamp: Date.now() });
  saveData(); res.json({ success: true });
});
...
app.post('/publish-intcom', (req, res) => {
  ...
  if (!tags || tags.length === 0) return res.json({ error: '请至少选择一个标签' });
  data.intcomArticles.push({ id: generateUniqueId(), author: user.nickname, title, content, tags, recommends: 0, timestamp: Date.now() });
  saveData(); res.json({ success: true });
});
...
app.post('/upload-video', (req, res) => {
  ...
  data.videos.push({ id: generateUniqueId(), author: user.nickname, authorId: userId, title, url, desc: desc || '', likes: 0, favorites: 0, comments: [], timestamp: Date.now() });
  saveData(); res.json({ success: true });
});
